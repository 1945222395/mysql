...
nfs_path="/srv" # 可能需要修改
	...
	mount -t nfs ${nfs_ip}:${nfs_dir} ${nfs_path}
...
app_name=jboot
app_bak='/app/app_bak'
app_start='/etc/init.d/jboot start' 
app_stop='/etc/init.d/jboot stop' 
url='localhost:80'
...
APP_update () {
	...
	if [ $? -ne 0 ];then 
		cd ${nfs_path}/${project_name};cp -a ./* ${app_path}/${app_name}
	#================================================================================
	# 初始化操作
		\cp -rf /app/crowd/tmp/webRoot/jboot.properties /app/crowd/jboot/webRoot/
		\cp -rf /app/crowd/tmp/webRoot/cron4j.properties /app/crowd/jboot/webRoot/
		\cp -rf /app/crowd/tmp/bin/* /app/crowd/jboot/bin/
		cd /app/crowd/jboot/bin/; chmod +x ./*
		cd /app/crowd/jboot/webRoot/; rm -rf upload
		cd /app/crowd/jboot/webRoot/; ln -s /app/bigData/upload ./
	#=====================================================================================
