...
ls "${war_data}"/"${JOB_HOME}" &>/dev/null
if [ $? -eq 0 ];then echo -e "\n\n\t当前最新war包为:  ${war_path}\t\n\n";else echo -e "\n\n\t无法搜索到${war_name}相关war包,发布异常即将退出\t\n\n";exit;fi
# global
#=============================
global_env () {
...
if [ ${judge} -eq 3 ];then
	...
	cd ${init_app_path}; tar zcf ${war_name}.bak${Date}.tgz   ./${war_name}*
	if [ $? -eq 0 ]
	then 
		cd ${init_app_path};mv ${war_name}.bak${Date}.tgz ${war_bak_path};
		#cd ${init_app} ;rm -f current
		cd ${init_app_path}; rm -rf ${war_name}*  && echo -e "\n\n---------->\t${Date}\t<-------\n==::))\t${war_name}备份成功...\t((::==";
	else 
...
init_war_deploy () {
	...
	cd ${init_app_path};[[ -f ${war_all_name} ]] && echo -e "\n\n\t---->\t当前更新war包版本为:  ${war_version}\t<----\n\n"
	cd ${init_app_path}; unzip ${war_all_name} -d ${war_name} &>/dev/null && echo -e "\n\n\n unzip ${war_all_name}:  [\tOK\t]\n\n";
	#if [ -f ${init_app}/current ];then
	#	echo -e "\n\n\n*******\ttomcat初始化失败,请重试\t*********\n\n"; rm -f ${init_app}/current;exit
	#else 
	#	ln -s ${init_app_path}/${war_version} ${init_app}/current
	#fi
	ps -ef |grep ${TOMCAT_HOME} | grep -v grep | awk '{print $2}' >  /usr/local/bin/pid
...
#开始清理最新数据
	#cd ${init_app};rm -f current
	cd ${init_app_path}; rm -rf ./${war_name}*
#开始恢复备份数据
