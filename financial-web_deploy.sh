...
confirm_action="${3}"
Rollback_version="${4}"
zip_path="${zip_data}/${job_name}/webZip"
...
init_app_path="/app/webapps/current/WEB-INF/classes"
...
if [ ${judge} -gt 0 ];then
	...
	then 
		...
		cd ${init_app_path}; rm -rf com com*zip && echo -e "\n\n---------->\t${Date}\t<-------\n\==::))\t${old_zip_name}备份成功...\t((::==";
	else 
...
init_zip_deploy () {
	...
	cd ${init_app_path}; unzip ${zip_all_name}  &>/dev/null && echo -e "\n\n\n unzip ${zip_all_name}:  [\tOK\t]\n\n";
}
tomcat_restart () {	
...
#开始清理最新数据
        cd ${init_app_path};
        cd ${init_app_path}; rm -rf com com*zip
#开始恢复备份数据
...
update)
        Clean_backups && init_zip_backup && init_zip_deploy &&  Post_steps
;;
rollback)
